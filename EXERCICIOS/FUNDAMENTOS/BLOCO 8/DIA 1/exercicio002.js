/* 2 - Desenvolva uma HOF que retorna o resultado de um sorteio.

Esta HOF irá gerar um número aleatório entre 1 e 5 recebendo como parâmetros o número apostado e 

uma função que checa se o número apostado é igual ao número sorteado.

O retorno da sua HOF deve ser uma string (Ex: "Tente novamente" ou "Parabéns você ganhou"). */

const sortMatch = (numberUser, numberLot) => {
  if (numberUser === numberLot) {
    return true;
  } 
  return false;
};

const loteria = (numberUser, funcComparaResults) => {
  let numberSort = Math.round(Math.random()* 5);
  console.log(numberSort);
  if(funcComparaResults(numberUser, numberSort)) {
    return 'Parabéns você ganhou';
  }
  return 'Tente novamente';
};

console.log(loteria(5, sortMatch));